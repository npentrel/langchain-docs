...
A bake base is the starting image for the bake that produces the recipe snapshot. With no bake base, LangSmith's default sandbox template is the starting point. To start from something else, set exactly one of these:
