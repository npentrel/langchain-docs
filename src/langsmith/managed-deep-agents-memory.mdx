...
Normally, a managed deep agent's conversational memory is scoped to a thread or session. Durable memory is optional knowledge that an agent can retain across threads and sessions. Managed Deep Agents (MDA) do not have durable memory by default.
...
Use durable memory for knowledge the agent should learn while it runs and reuse across threads. For always-on behavior, use [instructions](/langsmith/managed-deep-agents-instructions) instead. For task-specific procedures, use [skills](/langsmith/managed-deep-agents-skills) instead.
...
The agent reads and updates memory with the built-in [`read_file`](/oss/deepagents/tools#built-in-harness-tools), [`edit_file`](/oss/deepagents/tools#built-in-harness-tools), and [`write_file`](/oss/deepagents/tools#built-in-harness-tools) tools.
