...
To enable [human-in-the-loop](/oss/langchain/human-in-the-loop) with `interrupt()`, we need to compile with a [checkpointer](/oss/langgraph/persistence) to save state between runs:
...
<CardGroup cols={2}>
  ...
  <Card title="Human input is first-class" icon="user" href="/oss/langchain/human-in-the-loop">
...
<CardGroup cols={2}>
  <Card title="Human-in-the-loop patterns" icon="user-check" href="/oss/langchain/human-in-the-loop">
